...
\begin{align*}
\Id &: \pair{\fst}{\snd} = \Id \\
\text{\it DistrPair} &: \pair{f}{g}\circ h = \pair{f \circ h}{g \circ h}
\end{align*}
...
\begin{array}{lcll}
\pi_{E} (n) & =&  
\pi_{E'}(n)\circ \fst & \text { if } n > 1 \text{ and } E = E' \times A\\
&& \snd & \text{ if } n= 1
\end{array}
\]
...
\begin{align*}
{\text{\it ExAbs}} & :\, \App \circ (\abs f \times \Id) = f\\
{\text{\it UniAbs}} &:\, \abs(\App \circ (f \times \Id)) = f
\end{align*}
...
{\text{\it Red}}_1 & : \App \circ \la \abs f \circ y, x\ra = f\circ\la y, x\ra \\
{\text{\it Red}} & : \App \circ \la\abs f, x \ra = f\circ\la \Id , x\ra \\
{\text{\it DistrAbs}} & : \abs f \circ g = \abs(f \circ (g \times \Id ))
\end{align*}
We can also show that abstraction is a bijection between the arrows of A×B →C and those
of $A \to B \impl C$, with inverse:
$$
{\abs}^{-1}\, f = \App \circ \la f \times \Id \ra
$$
...
as:
$$
f \impl g = \abs(g\circ \App \circ (\Id \times f)).
$$
...
F_E ((M\, N)) &= \App \circ \la F_E (M) , F_E (N) \ra
\end{align*}
...
$$
F_E (M) = \abs \abs (\App\circ \la \snd \circ \fst, \App \circ \la \snd \circ \fst, \snd\ra \ra) : 1 \to A.
$$
