...
\IdOne &: \Id = \Nil
\end{align*}
Again, $\IdOne$ does not identify every $\Id$ with every $\Nil$,
but only (restoring explicit types) $\Id_1$
with $\Nil_1$. Now it may be checked that $\Unii$ is actually a
consequence of $\text{\it Zero}$
and $\IdOne$. The rule $\text{\it Zero}$ is a bona fide rewrite rule, 
which leaves the special equality $\IdOne$ to be dealt with in an ad hoc
fashion.
...
is $\impl$-elimination.%
\footnote{Editor's note: In the original paper Huet calls this ``$\impl$-introduction'', but it seems to me that this can't be right, so I changed it to the more standard current terminology}.
...
\footnote{This is ``$\impl$-elimination'' in the original paper,
but since I changed it above I also have to change it here.}
and plays somewhat the role
...
\end{align*}
We can also show that abstraction is a bijection between the arrows of $A \times B \to C$ and those
of $A \imp B \impl C$, with inverse:
$$
{\abs{}}^{-1}\, f = \App \comp (f \times \Id).
\footnote{Changed angle brackets to parens here. The brackets make no sense for this formula. But the rule
for {\it UniAbs} implies this definition.}
$$
...
M \comptr M' & \Longrightarrow \abs{A}M \comptr \abs{A}M' \tag{$\xi$}\\
M \comptr M' & \Longrightarrow (M \, N) \comptr (M' \, N)\\
M \comptr M' & \Longrightarrow (N \, M) \comptr (N \, M')
\end{align*}
...
\end{array}
\]
and thus that $R^i_1(M) = M \comp \rho(i)$. As a particular case we get $M^{+1} = M\comp \fst$ 
...
various $\l$-calculi can be found in Curien’s extensive monography \cite{6}.
...
\bb{19} J. Lambek and P. J. Scott, “Aspects of Higher Order Categorical Logic.” Contemporary
