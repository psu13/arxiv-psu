...
in order to complete $\R$ to a {\it canonical} system [17,9].
...
$$
F_E (M) = \abs \abs (\App\comp \la \snd \comp \fst, \App \comp \la \snd \comp \fst, \snd\ra \ra) : 1 \imp A.
$$
...
\begin{align*}
([A]M\, N) & \comptr M\{N\} \tag{$\beta$}\\
M \comptr M' & \impl [A]M \comptr [A]M' \tag{$\xi$}\\
...
and thus we can read the law {\it UniAbs} as $[x](M^{+1}\, x) = M$. 
...
basis for an eﬃcient implementation of the language [3].

\subsection{The CCC Word Problem}

The {\bf Exp} theory above is decidable [27]. Unfortunately, no canonical system is known for the full
...
checked by the Knuth-Bendix decision procedure [9]. Note that this is possible because a system
...
We remark that Klop has shown [16] that $\l$-calculus with surjective pairing
...
$\l$-calculus in the manner of Scott, as described in [25].
...
noticed the close connection between the categorical operators of Lambek [18] and de Bruijn’s
nameless notation for $\l$-calculus [2]. Instrumental to this was a close study of the ML compiler
developed by Lockwood Morris for the LCF project [8]. The syntactic theory from 4.3 is similar to
the formalization of Automath by Jutting [14], except that the absence of dependent types makes
things a little simpler. This treatment of $\l$-calculus computation was the basis of the author’s
implementation in ML of the Calculus of Constructions [4]. The author acknowledges numerous
