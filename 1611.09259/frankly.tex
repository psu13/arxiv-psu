...
\newcommand{\key}[1]{\textbf{#1}} % keyword
...
%% \slab{definitions}     &e       &::=& \many{r} \mapsto n \mid e \mid () 
\\[1ex]
...
%% The $e \mid e'$ construct composes the clauses of $e$ with those
...
\inferrule[Handle]
  ...
  {\checksgs{B'}{\key{handle}^\adj_{\effbox{\sigs}B'}~ m ~\key{with}~
               (\handle{c~\many{x}}{z} \mapsto n)_c \mid
                x \mapsto n'}}
%% \inferrule
%%   {\infers{\Gamma}{\sigs \oplus \adj}{m}{A} \\
...
%%                (\handle{c~\many{x_c}}{g_c} \mapsto n_c)_c \mid
%%                 x \mapsto n'}}
...
%% %%           (\handle{c~\many{x_c}}{g_c} \mapsto M_c)_{c \in \sigs} \mid
%% %%           x        \mapsto M
%% %% \end{equations}%
%% \begin{equations}
%% M &::= & \many{e} \\
...
(\textrm{evaluation contexts})\quad \EC
  ...
  &\mid& \key{handle}^\adj_G~ \EC ~\key{with}~
            (\handle{c~\many{x_c}}{z_c} \mapsto n_c)_c \mid
            x \mapsto n' \\
...
\key{handle}^\adj_G~v~\key{with}~(\handle{c~\many{x_c}}{z_c} \mapsto n_c)_c \mid x \mapsto n'
  &\reducesto& n'[v / x] \\
\key{handle}^\adj_G~\EC[c'~\many{w}]~\key{with}~(\handle{c~\many{x_c}}{z_c} \mapsto n_c)_c \mid x \mapsto n'
...
\var{HC}(\key{handle}^\adj_G~ \EC ~\key{with}~
            (\handle{c_i~\many{x_{c_i}}}{z_{c_i}} \mapsto n_{c_i})_i \mid
            x \mapsto n') =\quad \\
