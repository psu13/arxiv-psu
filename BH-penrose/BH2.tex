...
the vacuum states of the fields appear to be empty near $\scri^-$ these same states
will appear to ``contain'' particles when observed at  $\scri^+$ because of effects 
related to the curvature of spacetime. The details of the calculation trace
wave packets from $\scri^+$ back to $\scri^-$ 
...
\newpage
\noindent
A more minimal one without the coordinates:
...
\begin{tikzpicture}[scale=5]
    ...
    \node[above right] at (1,.5) {\small$\scri^+$};
    \node[below right] at (.8,-.15) {\small$\scri^-$};
    \node[below, rotate=45] at (0.4,.4) {\small ${\H}^+$};
    %\node[below, rotate=45] at (0.4,.4) {\small Event Horizon};
