...
(\cite{Renormalization},\cite{Renormalization-advanced}).
...
intimidating. In this article, we will give a bare bone of this program and
refer interested readers to more advanced literature (\cite{BPH}%
,\cite{Zimmermann}). Note that we need to use some regularization procedure
...
will have this property that all divergences can be absorbed into redefinition
of few physical parameters. Those which have this property are called
$renormalizable$ theories and those which don't are called unrenormalizable
theories. This has became an important criteria for choosing a right theory
because we do not really know how to handle the $unrenormalizable$ theory.

\section{Renormalization Schemes}

There are two different methods to carry out the renormalization program, i)
conventional renormalization which is more intuitive but mathematically
complicated, ii) BPH renormalization which is simple to describe but not so
...
the $substraction$.\newline\underline{\textbf{Mass and wavefunction
renormalization}}
...
This is the $mass$ $renormalization.$ Since $\Sigma\left(  \mu^{2}\right)  $
...
$wavefunction$ $renormalization$ $constant$. Thus another divergence is
...
in Eq (\ref{finite-part},\ref{finite part 2}), called the $renormalization$
...
This is the basic idea behind the $renormalization$ $group$ $equations$
(\cite{Renor Group}).\newline\underline{\textbf{Coupling constant
renormalization}}
...
multiplicative constants, is called being $multiplicatively$ $renormalizable$.
...
that in Eq ( \ref{counterterm}).\newline\underline{\textbf{Power Counting
Method}}
...
renormalization.\newline\underline{\textbf{More on BPH\ renormalization}}
...
\begin{enumerate}
\item \underline{\textbf{Convergence of Feynman diagrams}}
...
\item \underline{\textbf{Classification of divergent graphs}}\newline It is
useful to distinguish divergent graphs with different topologies in the
construction of counterterms.

\begin{enumerate}
\item \underline{Primitively divergent graphs}\newline A primitively divergent
...
\item \underline{Disjointed divergent graphs}\newline Here the divergent
...
\item \underline{Nested divergent graphs}
...
\item \underline{Overlapping divergent graphs}\newline These diagrams are
...
literature (\cite{BPH}.\cite{Zimmermann}).
\end{enumerate}
\end{enumerate}
...
\]
is called the $index$ $of$ $divergence$ of the interaction. Using the fact
...
\begin{enumerate}
\item $\delta_{i}<0$\newline In this case, $D$ decreases with the number of
i-th type of vertices and the interaction is called $super-renormalizable$
...
\item $\delta_{i}=0$\newline Here $D$ is independent of the number of i-th
type of vertices and interactions are called $renormalizable$ $interactions$.
...
\item $\delta_{i}>0$\newline Then $D$ increases with the number of the i-th
type of vertices and all Green's functions are divergent for large enough
$n_{i}.$ These are called $non-renormalizable$ $interactions.$ There are
...
$canonical$ $dimension$ which is defined in terms of the high energy behavior
...
non-renormalizable.\newline\underline{\textbf{Counter terms}}
...
{\textbf{Theories with vector fields}}
...
\begin{enumerate}
\item \underline{Massless vector field}\newline Massless vector field is
...
{\textbf{Renormalization of composite operators}}\newline Superficial degrees
...
\]%
\begin{equation}
\quad\quad\quad=Z_{\Omega}Z_{\phi}^{n/2}G_{lR}^{(n)}(x;x_{1},...x_{n})
\label{Composite}%
\end{equation}
...
\begin{enumerate}
\item \underline{\textbf{Broken symmetry and renormalization}}\newline For the
...
known as the $Szymanzik$ $theorem$ (\cite{Szymanzik}). Note that for the soft
...
\item \underline{\textbf{Ward Identity}} (\cite{Ward})\newline In case of
...
0\right\rangle \\
&  +i\int d^{4}xe^{-ip\cdot y}\left\langle 0\left\vert T\left(  \phi\left(
y\right)  \phi^{\dag}\left(  0\right)  \right)  \right\vert 0\right\rangle
\end{align*}
...
p\right)  \label{Ward}%
\end{equation}
This is example of Ward identity (\cite{Ward}).\newline This relation is
