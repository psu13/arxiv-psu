...
\ees
\bes
A \sland B \xto{\pAB} A,\,\,A \sland B \xto{\ppAB} B, \quad
\inferrule{C \xto{f} A\qquad C \xto{g} B}{C \xto{\la f, g\ra} A \sland B}
\tag{3}
\ees
\bes
...
$$
\inferrule{A \sland B \xto{\ppAB} B\quad A \sland B \xto{\pAB} A}{A \sland B \xto{\la \ppAB,\pAB\ra} B\sland A}
$$
The label $\la \ppAB,\pAB\ra$ appearing in the last line may be used
...
\item[($3'$)] $\pAB\la f,g \ra = f$, $\ppAB\la f,g \ra = g$,
$\la\pAB h, \ppAB h\ra = h$, for all $f:C \to A$, $g:C \to B$, and $h: C\to A\sland B$;
\item[($4'$)] $\ev_{A,B}\la h^*\pr{C,B}, \prp{C,B}\ra = h$,
$(\ev_{A,B}\la k^*\pr{C,B}, \prp{C,B}\ra)^* = k$,
...
\pdfbookmark{Functional Functional}{completeness}
\begin{theorem}[Functional Functional of Cartesian closed categories]
For every polynomial $\varphi(x):B \to C$ in an indeterminate $x: 1 \to A$
there is a unique constant $f: A \times B \to C$ such that $f\bracket{x0_B,l_B} = \varphi(x)$.
\end{theorem}
\begin{proof}
