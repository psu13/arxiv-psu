...
\def\scop{\S^{\C^{op}}}
\def\pr{^\prime}
\def\sland{\mathrel{\scriptstyle \land}}
\def\PU{\mathcal P U}
\def\P{\mathcal P}
\def\UU{(U\to U)}
\def\BA{B \to A}
...
\calA \to \calB$ and every element $b\in |\calB|$, there exists a unique homomorphism $f\pr : \calA[x] \to
\calB$ such that $f\pr h_x = f$ and $f\pr (x) = b$. In the special case when $\calB = \calA$ and $f$
is the identity homomorphism $\calA \to \calA$, $f\pr$ is the {\it substitution} homomorphism
...
exists a unique homomorphism $h\pr: \calA[x] \to \calA$ such that
...
\begin{enumerate}
\item[(0)]$\qquad\inferrule {a \in A \qquad f \in B \from A}{f \app a \in B}$,
\end{enumerate}
...
\ees
\bes
A \sland B \xto{\pi_{A,B}} A,\,\,A \sland B \xto{\pi\pr_{A,B}} B, \quad
\inferrule{C \xto{f} A\qquad C \xto{g} B}{C \xto{\langle f, g\rangle} A \sland B}
\tag{3}
\ees
\bes
(A\from B) \sland B \xto{\eval_{A,B}} A, \qquad \inferrule{C\sland B \xto{h} A}{C \xto{h^*} (A \from B)}
\tag{4}
\ees
The labels are useful in naming proofs. For example, the commutative law is proved thus:
$$
\inferrule{A \sland B \xto{\pi\pr_{A,B}} B\quad A \sland B \xto{\pi_{A,B}} A}{A \sland B \xto{\langle \pi\pr_{A,B},\pi_{A,B}\rangle} B\sland A}
$$
The label $\langle \pi\pr_{A,B},\pi_{A,B}\rangle$ appearing in the last line may be used
