...
$$
\inferrule{A \sland B \xto{\prpAB} B\quad A \sland B \xto{\prAB} A}{A \sland B \xto{\la \prpAB,\prAB\ra} B\sland A}
$$
The label $\la \prpAB,\prAB\ra$ appearing in the last line may be used
...
\item[($3'$)] $\prAB\la f,g \ra = f$, $\prpAB\la f,g \ra = g$,
...
\kxa k (\psi(x),\chi(x))&= \kxa\chi(x)\bracket{\prAB, \kxa\psi(x)}\tag{iv}\\
...
This third step is a bit mysterious, but remember that
