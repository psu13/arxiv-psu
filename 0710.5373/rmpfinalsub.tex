...
f_B(t,{\bf x})p_B(t,{\bf x}')\bigl[\hat{\phi}(t,{\bf x}),\hat{\pi}(t,{\bf
x}')\bigr]
...
\begin{equation}
\hat{\Phi}(t,z) = \int_{0}^\infty \frac{dk}{\sqrt{4\pi k}} \bigl(
\hat{b}_{-k}e^{-ik(t-z)} + \hat{b}_{+k}e^{-ik(t+z)}
+ \hat{b}_{-k}^\dagger
e^{ik(t-z)} +
\hat{b}_{+k}^\dagger e^{ik(t+z)}\bigr).
\end{equation}
...
\begin{multline}
\hat{\Phi}_{+}(V) = \int_0^\infty d\omega\,\bigl\{\theta(V)
[\hat{a}_{+\omega}^R g_{\omega}(v) +
\hat{a}_{+\omega}^{R\dagger}g_{\omega}^*(v) ] \\
 + \theta(-V) [\hat{a}_{+\omega}^L  g_{\omega}(\bar{v}) +
\hat{a}_{+\omega}^{L\dagger} g_{\omega}^*(\bar{v}) ]\bigr\},
\end{multline}
...
x}_\bot}\nonumber \\
& \quad\quad \times e^{i(k_0t-k_z z)\cosh\alpha a}
...
\begin{equation}
t(\tau ) = \frac{\sinh a \tau}{a}, \;\;\; z(\tau) = \frac{\cosh a \tau}{a}\;
...
R^\perp & = \frac{|q|^2}{16\pi^3 T} 
             ...
             e^{i\Delta E (\tau' - \tau'')}     
\nonumber \\   
& \quad \times e^{ i \omega [\sinh a\tau' - \sinh a\tau'']/a }
...
\begin{equation}
ds^2= \left( 1 - 2M/r \right) dt^2 - \frac{1}{( 1 - 2M/r)} dr^2.
\label{SS}
\end{equation}
...
- \frac{4\sin^2(\omega+\Delta E)T}{\alpha^2 + (\omega+\Delta E)^2}\nonumber \\
 & \quad +\frac{4\alpha^2\cos 2(\omega +\Delta E) T}
                   {[\alpha^2+(\omega+\Delta E)^2]^2} \nonumber \\
 & \quad +\frac{4\alpha^3\sin 2(\omega+\Delta E)T}
...
\begin{figure}
\begin{center}
\mbox{\epsfig{file=crispinoetal_fig04.ps,width=4in,angle=0}}
\end{center}
...
K_{i\omega/ a} 
[ ({k_\perp}/{a}) e^{a\xi } ]
\,e^{i{\bf k}_\bot\cdot{\bf x}_\bot - i \omega \tau}.
\label{16.5}
\end{equation}
...
\begin{figure}
\begin{center}
\mbox{\epsfig{file=crispinoetal_fig09.ps,width=4in,angle=0}}
\end{center}
