...
if the entanglement only appears after $t_{\it scramble}$ one finds compatibility
...
\begin{equation}
P_{\it fail}\leq2^{k}2^{-s}\label{eq:pfail-1}
\end{equation}
...
\begin{equation}
\rho_{\it max}=\frac{1}{2^{k}N}\left(\begin{array}{cccc}
\mathbb{I}_{N} & 0 & \cdots & 0\\
...
\begin{equation}
\mathcal{O}_{\it short}=\left(\begin{array}{cccc}
\mathbb{I}_{N} & 0 & \cdots & 0\\
0 & 0 & \cdots & 0\\
\vdots & \vdots & \ddots & 0\\
0 & 0 & 0 & 0
...
find 
\begin{eqnarray*}
\mathrm{Tr}\rho_{\it BhA}\times\mathcal{O}_{\it short} & = & 1\\
\mathrm{Tr}\rho_{\it max}\times\mathcal{O}_{\it short} & = & 1/2^{k}
\end{eqnarray*}
so we see the answer is highly sensitive to the state of the stretched
horizon. The danger is that operators of the form \foreignlanguage{english}{$\mathcal{O}_{\it short}$}
...
scrambling time $t_{\it scramble}$. If, however, the stretched horizon
...
coefficient vanishes, the information is retained for a time $t_{\it scramble}$,
...
a neighboring cell after $t_{\it scramble}$$ $ and assume that this
