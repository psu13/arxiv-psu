...
one of the main goals of category theory: it characterizes universal facts
...
getting to the end of our ``game'' as fast as possible. Clearly this will be no substitute
for really learning the subject. Any of the references listed at the end will be a good
place to start to better understand the whole game.
...
\goodbreak
I'm going to break with well established tradition in the mathematical literature and
fully mostly spell out names for clarity rather than engaging in the strange and random
abbreviations that I see in most category theory texts. The general fear of readable names 
in the mathematical literature is fascinating
to me, having spent most of my life trying to think up readable names in program source
code. Life is too short to deal with names like ``Ob'', or $\cat{Htpy}$, or $\cat{Matr}$.
Also, for this note the only specific
category that we will run into is $\Set$, where the objects are sets and the arrows
are mappings between sets.

Speaking of sets, in the definition of categories we were careful about not calling anything
...
\end{defn}
\ni
You will also see people write $\Hom(F, G)$, $\Hom_{\CC}(F,G)$ or even $\CC(F,G)$ for this in the context
a functor category $\CC$.
...
Thus, we have worked out that the value of $\Arrows(X,-)$ at $f$ 
should be the arrow $f \circ -$. Sometimes you will see this written
$\Arrows(X, f) = f \circ -$, which I find a bit odd because now we are
overloading the kinds of things that can go into the ``$-$'' slot.
...
Our next jump in abstraction is to talk about duality. Duality in mathematics comes up in a lot of
...
\end{itemize}
To write this in the dual language, you just change $\Arrows(X, -)$ to $\Arrows(-, X)$, which
...
Your reward for having climbed with me all the way the abstraction ladder to get to our result
is yet more abstraction!

Suppose you are given an object $Y$ and you apply the Yoneda lemma
by substituting $\Arrows(Y,-)$ for the functor $F$. Then
$$
...
$$
Each of the functors $\Arrows(-, X)$ maps from $\CC^\op \to \Set$ because
that's how we defined the represented functors. So now let's jump one more level of
abstraction and define a functor that maps objects to the functors that they represent,
and arrows to the natural transformations between those functors.
Given an object $Y\in\CC$ define
...
abstracted over all the objects and arrows. Thus our abstraction ladder is again one level
higher.  Also, you have your
choice of writing this mapping as $\yo:\CC \to \Fun(\CC^\op, \Set)$ or
$\yo:\CC^\op \to \Fun(\CC, \Set)$. All that changes are the order of the arguments.

The Yoneda lemma then can be used to prove that these mappings are naturally invertible, so $\yo$
completely characterizes, or {\it embeds}, the category $\CC$ inside the functor
category $\Fun(\CC^\op, \Set)$. Thus $\yo$ is called the {\it Yoneda embedding},
and you can read about the rest of the details in the references.

Now we know why people say things like ``Every object
in $\CC$ can be understood by understanding the maps into (or out of) it.''
More precisely:
\begin{cor}
Let $\CC$, $X$, and $Y$ be given as above.
\begin{itemize}
\item $X$  and $Y$ are isomorphic if and only if for every object $A \in \CC$, 
the sets $\Arrows(X, A)$ and $\Arrows(Y, A)$ are naturally isomorphic.
\item $X$ and $Y$ are isomorphic if and only if the functors that they represent are naturally isomorphic.
In particular, if X and Y represent the same functor then they must be isomorphic.
\end{itemize}
\end{cor}
...
$\Fun(\CC, \DD)$ & Functor category between $\CC$ and $\DD$. Also $[\CC,\DD]$ or $\DD^\CC$.\\
$\Nat(F, G)$ & The collection of natural transformations from $F$ to $G$. 
Also written $[\CC,\DD](F,G)$, or~$\mathop{\mathit {Nat}}(F,G)$ or just~$\Hom(F,G)$. \\
