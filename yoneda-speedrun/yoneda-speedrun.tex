...
applied to $f$ is an arrow that maps $\Arrows(X,A) \to \Arrows(X,B)$ that
we will call $f_*$.  
If $g: X \to A$ is in $\Arrows(X,A)$
then the value 
that we want for $f_*$ at $g$ is 
is $f_*(g) = (f \circ g): X \to B$.
...
$\Arrows(X,-)(f) = \Arrows(X,f) = f \circ -$ to maximize the use of different placeholder symbols.
...
switches the direction of all the arrows and the order of composition.
