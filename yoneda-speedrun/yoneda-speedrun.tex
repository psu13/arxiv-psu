...
$\cat{Measu}$ or $\cat{Htpy}$ or $\cat{Matr}$ and the examples will little sense unless you are
already familiar with the subject from which the category comes.
...
is that I'm going to give you one more surprise step up in abstraction!

Suppose you apply the Yoneda lemma, and you make the 
functor $F$ in the statement the represented 
functor for some object $Y$ in $\CC$. Then
$$
\Nat(\Arrows(X, -),\Arrows(Y,-)) \iso \Arrows(Y,-)(X) = \Arrows(Y,X)
$$ or
$$
\Arrows(X,Y) \iso \Nat(\Arrows(Y, -),\Arrows(X,-))
$$
Note the order of the arguments! We can also write:
$$
\Arrows(X,Y) \iso \Nat(\Arrows(-, X),\Arrows(-,Y))
$$
Now the idea is to use this fact to define a new functor $\yo: \CC \to \Arrows(\CC^\op, \Set)$ that maps
objects to arrow functors and arrows to their {\it pre}-composition map. So for each object $X$ and arrow $f$ define
$$
\yo(X) = \Arrows(-, X) : \CC^\op \to \Set
$$
and
$$
\yo(f) = \Arrows(-, f) = f^* = - \circ f 
$$
Notice the direction of the arrows! 
Using the lemma we can show that this functor is an {\it embedding} 
of $\CC$ inside $\Arrows(\CC^\op, \Set)$, which amounts to meaning
that both mappings above are invertible. The functor $\yo$ is then called
the {\it Yoneda} embedding. Of course you can define this in the dual way too
if you want.

This is the origin of statements like ``The Yoneda lemma says that every object
in $\CC$ can be understood by understanding the maps into (or out of) it.''.
This notion can be made precise by proving the following corollary, which I will not do here
because I'm just a poseur.
\begin{cor}
...
Two last thoughts. First, I wish I could think of a more meaningful notation than $\Arrows(X, -)$ for
the arrow functor. But oh well, no one else can either.

Second, I need to shout out the 
