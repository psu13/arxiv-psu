...
\goodbreak
For this note the only specific
...
$\Fun(\CC,\DD)$, or even better: $\Arrows(\CC, \DD)$, since functors are just a kind of arrow.
So there.
...
$$
If you compare 
this to the definition of the represented functors you can see we are
to varying all the objects that used to be fixed. So this is yet another
level of wrapping all the arguments in that definition. My head hurts.

The Yoneda lemma can be used to prove that these mappings are naturally invertible, so $\yo$
completely characterizes, or {\it embeds}, the category $\CC$ inside the functor
category $\Arrows(\CC^\op, \Set)$. Thus $\yo$ is called the {\it Yoneda embedding},
and you can read about the rest of the details in the references.

This embedding says something like ``Every object
...
$\Fun(\CC, \DD)$ & The functor category between $\CC$ and $\DD$. Same as $\Arrows(\CC,\DD)$ if you want. Also written $[\CC,\DD]$ or $\DD^\CC$.\\
