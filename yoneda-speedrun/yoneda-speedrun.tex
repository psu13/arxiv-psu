...
denote the value of the map at some other object $Y$ (or arrow $f$) we will write 
$\Arrows(X,Y)$ (or $\Arrows(X,f)$). This is a bit of an abuse of notation since we 
...
we call a functor defined this way the functor {\it represented} by and object $X$.
...
First, I wish I had been able to think of a better notation for the ``arrow functor'' that 
does not use the ``$-$'' placeholder. Trying to do so makes defining the mapping
in the lemma awkward since you now have to keep track of multiple dummy variables. 
...
Maybe short for {\it Hom}? Who knows. I thought about
using a $\lambda$-calculus style notation but that would probably be too weird for people.
