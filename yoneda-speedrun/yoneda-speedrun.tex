...
$\Fun(\CC,\DD)$ or $\Arrows(\CC, \DD)$. So there.
...
$$
Each of the functors $\Arrows(-, X)$ is in the category $\Fun(\CC^\op, \Set)$, because
that's how we defined the represented functors. So now let's jump one more level of
abstraction and define a functor {\it values} are these functors. That is define
...
\yo(f) = f^* = - \circ f .
$$
The Yoneda lemma can be used to prove that these mappings are invertible, so $\yo$
...
This result is the origin of statements like ``The Yoneda lemma says that every object
