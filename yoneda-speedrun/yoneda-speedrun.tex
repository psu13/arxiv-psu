...
a mapping with two arguments and an object $X$. Then we use the ``$-$'' symbol as a 
placeholder for the second argument. So then we write $\Arrows(X,Y)$ for
the value of the map at some other object $Y$. 
...
$\Arrows(X,-)(f) = \Arrows(X,f) = f \circ -$ to maximize the use of name overloading.
...
naming layer, so I did not like that either. Also, why is it called $H$? 
