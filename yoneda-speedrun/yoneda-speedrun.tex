...
going to define only the parts of the category theory ``lingo'' needed to explain what it
means. There are five or six layers of abstraction that I will try to explain and move
through as quickly and clearly as possible.
...
layer of naming (e.g. $\h_X$), which is the only other obvious choice. If I did do that, I'd use
a name besides $\h$ anyway. It seems like $R$, or $\mathop{\mathit{Rep}}$, or $\mathop{\mathit{Represented}}$ or something makes
much more sense. But, I'll just take comfort in the fact that no
