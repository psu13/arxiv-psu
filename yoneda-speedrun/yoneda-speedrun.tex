...
going to define only the parts of the category theory language needed to explain what it
means. Then we will restate the result in several ways at the end for perspective.

In the spirit of video game speedruns \cite{lobos}, we will skip entire interesting areas
of material in category theory in the name of getting to the end of our ``game'' as fast
...
\goodbreak
I have broken with well established tradition in the mathematical literature and mostly
...
$\mathop{\mathit{ob}}$, or $\cat{Htpy}$, or $\cat{Matr}$. Also, for this note the only
specific category that we will run into is $\Set$, where the objects are sets and the
...
transformations. Given that natural transformations look a lot like arrows,
...
\end{defn}
\ni
You will also see people write $\Hom(F, G)$, $\Hom_{\CC}(F,G)$ or even $\CC(F,G)$ for this
in the context a functor category $\CC$.
...
invertible implies that $\Nat(\Arrows(X, -),F)$ and  $FX$ are isomorphic (e.g.
...
\end{lemma}
\ni
Here $[\CCop, \Set]$ is the category of functors from $\CCop$ to $\Set$ and $\h_X$ is
$\Arrows(-,X)$. The notation $\pshf{\CC}(\h_X, F)$ denotes the arrows in the functor
...
\pg
Paolo Perrone \cite{Perrone} writes the contravariant version, and uses the standard term
"presheaf" for a functor from $\CCop$ to $\Set$.
...
natural transformations between those functors. Given an object $Y\in\CC$ define $\yo:\CC
\to \Fun(\CC^\op, \Set)$ by
$$
...
as $\yo:\CC^\op \to \Fun(\CC, \Set)$. All that changes is the order of the arguments in the
functors.

The Yoneda lemma can now be used to prove that these mappings are naturally invertible, so
$\yo$ completely characterizes, or {\it embeds}, the category $\CC$ inside the functor
...
points of view. So, I am grateful for all of the sources. 
...
letter (e.g. $\h_X$), which is the only other obvious choice. I'll take comfort in the fact that no
one else has done any better. At some point I'll probably make a version of this document
that uses one of the more standard systems of notation. But for now I like my verbosity.
...
Her book, \href{https://math.jhu.edu/~eriehl/context/}{\it Category
Theory in Context}, is also excellent \cite{Rhiel2016}. Recommended.
...
\bibitem{Leinster} Tom Leinster, \href{https://arxiv.org/abs/1612.09375}{Basic Category
Theory}, 2016.
...
Category Theory with examples from basic mathematics}.

\bibitem{Rhiel2016} Emily Rhiel, \href{https://math.jhu.edu/~eriehl/context/}{\it Category
Theory in Context}, Dover, 2016.

\bibitem{Rhiel2020} Emily Rhiel, \href{https://www.youtube.com/watch?v=SsgEvrDFJsM}{ACT
