...
$$
Now the idea is to use this fact to define a yet another 
new functor $\yo: \CC \to \Arrows(\CC^\op, \Set)$ that maps
objects to arrow functors and arrows to their {\it pre}-composition map. 
So for each object $Y$ and arrow $f$ define
$$
...
\yo(f) = f^* = - \circ f 
$$
Notice the direction of the arrows! This functor wraps up all of the represented
for all of the objects in $\CC$ into one mapping, and so is our last jump up
the abstraction chain.

Using the Yoneda lemma we can show that this functor is an {\it embedding} 
