%!TEX root = yoneda-speedrun-pal.tex
...
\newcommand{\h}{\textit{H}}                      % Homs (e.g. \h_A means Hom(-, A))
...
\date{\normalsize 2021-10-30 \\ {\footnotesize (Updated 2023-03-18)}}
...
\end{defn}%
\ni
To be maximally careful we should write $\Arrows_{\CC}(A, B)$ to mean the collection of all arrows 
from $A$ to $B$ in a category $\CC$.
But we will usually write $\Arrows(A,B)$ when it's clear what category $A$ and $B$ come from. People
...
Other notations for this functor include $\Hom(X, -)$, $\Hom_\CC(X, -)$, $\h^X$, ${\it h}^X$, and
...
functor and written $\CC(X,-)$, $\h^X$, $\mathop{\mathit{hom}_X}$, or $\Hom(X,-)$. \\
