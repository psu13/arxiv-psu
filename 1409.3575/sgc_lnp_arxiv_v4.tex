%%%%%%%%%%%%%%%%%%%% book.tex %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% sample root file for the chapters of your "monograph"
%
% Use this file as a template for your own input.
%
%%%%%%%%%%%%%%%% Springer-Verlag %%%%%%%%%%%%%%%%%%%%%%%%%%


% RECOMMENDED %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\documentclass[graybox,envcountchap]{svmono}
\documentclass[graybox,envcountchap,sectrefs,12pt]{svmono}

%\AtBeginDvi{\special{pdf: pagesize width 182truemm height 257truemm}}

% choose options for [] as required from the list
% in the Reference Guide

\usepackage{mathpazo}
\usepackage{helvet}
\usepackage{courier}
%
\usepackage{type1cm}         

\usepackage{makeidx}         % allows index generation
%\usepackage[dvipdfm]{graphicx}
%\usepackage[dvipdfmx]{graphicx}
\usepackage{graphicx}        % standard LaTeX graphics tool
                             % when including figure files
\usepackage{multicol}        % used for the two-column index
\usepackage[bottom]{footmisc}% places footnotes at page bottom

% see the list of further useful packages
% in the Reference Guide

%\makeindex             % used for the subject index
                       % please use the style svind.ist with
                       % your makeindex program

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%% Makoto's macro %%%

\usepackage{amsmath}
%\usepackage[fleqn]{amsmath}
\usepackage{amssymb,bm}
\usepackage{subfigure}
%\usepackage{mediabb}
\usepackage{color} % v4

\usepackage{mathrsfs} % \mathscrðg€ê

\graphicspath{{./fig_crop/}}
%\graphicspath{{./fig/}}

\newcommand{\PS}{{\scshape Post\-Script}}
\newcommand{\AmSLaTeX}{%
 $\mathcal A$\lower.4ex\hbox{$\!\mathcal M\!$}$\mathcal S$-\LaTeX}
\newcommand{\AmSLaTeXe}{%
 $\mathcal A$\lower.4ex\hbox{$\!\mathcal M\!$}$\mathcal S$-\LaTeXe}

%\usepackage{atbegshi}
%\AtBeginShipoutFirst{\special{pdf:tounicode 90ms-RKSJ-UCS2}}
\usepackage[colorlinks=true,linktocpage=true,linkcolor=blue,citecolor=blue]{hyperref}
%\hypersetup{pdfauthor={Makoto Natsuume},pdftitle={AdS/CFT duality user guide}}
% hyperref ÌÇÝÝÍÈé×­ã
% Springer
%\renewcommand\UrlFont{\rmfamily}

\usepackage{macro_natsuume}
\def\nameofpaper{Springer} %% dummy ÌRg[V[NGXA{ } ÌgÍÈñÅà¢¢

%%% for draft %%%
%\usepackage[outer]{showlabels}
%\renewcommand{\showlabelfont}{\tiny}
%\newcommand{\margin}[1]{\marginpar{\tiny #1}}
% for Kindle
\newcommand{\margin}[1]{}%{\marginpar{\tiny #1}}

%%%

\newcommand{\head}[1]{\runinhead{#1}}
\renewcommand{\subsubsection}[1]{\runinhead[#1]}
%\renewcommand{\subsubsection}{\paragraph}
% for double space
%\renewcommand\baselinestretch{2}
%\renewenvironment{subequations}{}{}

%%%
\usepackage[papersize={6in, 9.5in}, left=.5in, right=.5in, top=.7in, bottom=.9in]{geometry}
\sloppy
\input{../helpers/cmrsum.tex}

\begin{document}

\author{Makoto Natsuume}
\title{AdS/CFT Duality User Guide}
\date{31 Aug 2016}
%\subtitle{-- Monograph --}
\maketitle

\frontmatter%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%\include{dedic}
%\include{foreword}
\include{preface}
\include{preface_arXiv}
%\include{acknow}

\tableofcontents

\include{acronym}


\mainmatter%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%\include{part}
	\input{Chap_intro}
	   \input{./refs/Chap_intro_ref}
	\input{Chap_BH}
	\input{Chap_BH_thermo}
	   \input{./refs/Chap_BH_thermo_ref}
	\input{Chap_QCD}
	   \input{./refs/Chap_QCD_ref}
	\input{Chap_string}
	   \input{./refs/Chap_string_ref}
	\input{Chap_AdS}
	\input{Chap_AdSBH}
	   \input{./refs/Chap_AdSBH_ref}
	\input{Chap_Wilson}
	   \input{./refs/Chap_Wilson_ref}
	\input{Chap_hydro}
	   \input{./refs/Chap_hydro_ref}
	\input{Chap_GKPW}
	   \input{./refs/Chap_GKPW_ref}
	\input{Chap_others}
%	   \input{Chap_others_app}
	   \input{Chap_others_app_arxiv}
	   \input{./refs/Chap_others_ref}
	\input{Chap_QGP}
	   \input{Chap_QGP_app2}
	   \input{./refs/Chap_QGP_ref}
	\input{Chap_GL}
	   \input{./refs/Chap_GL_ref}
	\input{Chap_CMP}
	   \input{./refs/Chap_CMP_ref}
	\input{Chap_exercises}
%	\include{biblio}
%\include{appendix}

%\backmatter%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%\include{glossary}
%%\include{solutions}
%\printindex

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\end{document}
