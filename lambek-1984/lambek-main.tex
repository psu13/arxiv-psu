...
\footnote{In their book Lambek and Scott add this: We define
$\Ker h \equiv \{ x \in A \mid \la x, \top \ra \in |h|\}$, and
$|\ker h| \equiv \{\la x,x \ra \in A \times A \mid \la x, \top \ra \in |h|\}$ where
$|h|$ is the graph of the morphism $h$. This is kind what you would expect but
even in their book they only make this clear in a later discussion, and apparently
no one else on the entire Internet ever uses this concept in this context.}
...
\forall_{x \in A} \pf(x) &\equiv \{ x \in A \mid \pf(x)\} = \{x \in A \mid \tau\}\\
...
Finally, entailment $p_1,\dots ,p_m \ent_X q$ in $L(\LC)$ is defined as follows. To
simplify matters, take $m=1$ and $X = \{x\}$, $x$ a variable of type $A$. Then
$\pf(x) \ent_X \psi(x)$ means:
\begin{quote}
For all objects $C$ and all arrows $a: C\to A$, if $fa \doteq \tau\, 0_C$
then $ga \doteq \tau\, 0_C$.
\end{quote}
Here we have used functional completeness to write $\pf(x) \eqdx fx$ and
$\psi(x) \eqdx gx$ with $f,g: A \to \OM$.
\end{example}

The description of $L(\LC)$ is now complete. 
