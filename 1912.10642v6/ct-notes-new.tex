...
% Other packages
\usepackage[papersize={7.0in, 10.0in}, left=.6in, right=.6in, top=.7in, bottom=1in]{geometry}
...
\begin{eg}[group theory]\label{BGrepr}
 Let $G$ be a group. We have seen (\Cref{perm}) that a functor $F:\cat{B}G\to\Set$ is a permutation representation of $G$, that is, a set $X$ (which is the image $F\bullet$ of the unique object $\bullet$ of $\cat{B}G$) equipped with an action of $G$ on $X$. When is this functor representable?
...
\begin{ex}[sets and relations]
 ...
 Assign now to each $(X,\simord)$ the quotient set $X/\simord$ obtained by identified all elements of $X$ which are related by $\sim$ (recall \Cref{quotient}). Assume $X/\simord$ equipped with the identity relation. Denote by $q$ the quotient map $(X,\simord)\to X/\simord$.
