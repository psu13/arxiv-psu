...
\usepackage{graphicx}
\usepackage{epsf}
\newcommand{\dessin}[4]{\begin{figure}{\refstepcounter{figure}\label{#3}}\end{figure}%
\vspace*{12pt}%
\begin{center}
...
\end{flushright}
\vskip 2.5 truecm
\centerline{\Huge{A Primer for Black Hole Quantum Physics}}
\vskip 1. truecm
...
hence their difference yields \hfil \\ $\bar A_+ (\omega, \tau)
...
\begin{equation}
\alpha_{k \omega } = 
\int dU { e ^{i k U}\over \sqrt{4 \pi k}} i {\lrpartial}_U
...
F (see Fig.~(\ref{collapse})).

 \noindent Whereas Kruskal coordinates can
...
value ($\langle T_{uu}(\infty,u)) \rangle$). 
...
\footnote{In a recent numerical calculation in a model which is analogous to the black hole, Unruh \cite{xxx} has shown that a severe modification of the dispersion relation $\omega(k)$ for $k$ greater than some threshold value in no way affects the thermal spectrum of Hawking emission.}. After all these latter
...
$  X_{\star}$ fixes the density in the star). 


The figures (\ref{fluxinf}) and (\ref{fluxsurf}) show the outgoing flux  
