...
$\l$-Calculus} originally published in R. Hindley and J. Seldin, 
...
think in terms of elements and that we are mapping $x \mto u(x)$ and $y \mto v(y)$.
The pointwise group product of the maps, namely, $x,y \mto  u(x) \cdot v(y)$ is a very
nice map $g : A^2 \to B$ in the ordinary sense --- but unless the group $B$ is abelian,
$g$ is not a homomorphism. %
%\footnote{2021 Note: See \url{https://en.wikipedia.org/wiki/Eckmann-Hilton_argument}}
...
special map $\zeromap_A: A \mto \bbone$. (The domain $\bbone$, intuitively, has just
one ``element''.)  Moreover the rule about maps is that $\zeromap_A$ is unique; that is
whenever $f : A \mto \bbone$, then we have the equation
$$f = \zeromap_A .$$
%
...
with the intuitive interpretation that it is the map $ f,x \mto f(x)$. In the second
...
of types is, I think, reasonably simple. {\it Each \ccc\ represents a theory of functions}. The
...
to $\l$-terms and equations; but others can use whatever logical means they like for discussing 
functions. However, if the logician {\it proves} in his higher-order theory that a
...
higher-order logic (say, by adding some new primitives), so that we could express {\it in the
logic} the axiom that {\it every} \ccc\  (a structure satisfying a simple first-order statement)
...
with showing how to accommodate a finite number of types. By the way, it should be remarked
that 4 and 5 hold for an arbitrary (small) category, so in particular we have the (known)
...
\bibitem[4]{4} Goldblatt, R., (1979). {\it Topoi: The Categorical Analysis of Logic},
