...
\def\PU{\mathcal P U}
\def\P{\mathcal P}
\DeclareRobustCommand{\mm}{\Assert}
...
is easy to check that the functors $H_A \times H_B$ and $H_{A \times B}$ are naturally isomorphic. We also have to do the same for $(H_A \to H_B)$ and $H_{A \to B}$. Consider an element of $(H_A\to H_B)_C$. It is a family of maps
$$
\phi_f : H_A(D) \to H_B(D)
$$
...
So, since $\C$ is a \ccc, we find $\L\phi_p(q) \in (H_{A \to B})_C$. In the
other direction, let $t : C\to (A \to B)$.
