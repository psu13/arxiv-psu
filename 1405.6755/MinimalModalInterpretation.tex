...
\documentclass[twoside,english,aps,superscriptaddress,nobibnotes,nofootinbib]{revtex4-1}
\pdfoutput=1
...
\usepackage[unicode=true,
 ...
 {hyperref}
\hypersetup{pdftitle={The Minimal Modal Interpretation of Quantum Theory},
 pdfauthor={Jacob A. Barandes, David Kagan},
 pdfsubject={Physics},
 pdfkeywords={physics, quantum mechanics, interpretations}}
...
\global\long\def\eqsbrace#1{\left.#1\qquad\qquad\qquad\right\}  }
...
more formally as the Von Neumann-Lüders projection postulate \cite{vonNeumann:1955mfqm,Luders:1950udzddm}---by
...
to make sense of thought experiments like Schrödinger's cat, Wigner's friend,
...
the Schrödinger equation. However, in traditional formulations of
...
obeys the Schrödinger equation; by contrast, this sort of cosmic assumption
...
outcome probabilities, and discuss possible corrections to the naïve
...
theory, including Schrödinger's cat, Wigner's friend, and the quantum Zeno paradox.
...
\begin{equation}
p\left(\cdot;t^{\prime}\given\left(\cdot;t_{1}\right),\left(\cdot;t_{2}\right),\dotsc\right)\colon\ \underbrace{p\left(q^{\prime};t^{\prime}\right)}_{\substack{\mathrm{epistemic}\\
...
the current time $t$.%
\footnote{Throughout this paper, we will always work in the so-called Schrödinger picture,
...
would naïvely lead to degenerate $2\times2$ reduced density matrices
...
\begin{equation}
\ket{\Psi_{\mathrm{EPR\hyphen Bohm}}}=\frac{1}{\sqrt{2}}\left(\left(1+\epsilon_{1}\right)\ket{\uparrow\downarrow}-\left(1+\epsilon_{2}\right)\ket{\downarrow\uparrow}+\epsilon_{3}\ket{\uparrow\uparrow}+\epsilon_{4}\ket{\downarrow\downarrow}\right),\quad\absval{\epsilon_{1}},\absval{\epsilon_{2}},\absval{\epsilon_{3}},\absval{\epsilon_{4}}\ll1.\label{eq:EPRPairStateVecDegenBreaking}
\end{equation}
...
unitary Schrödinger dynamics of closed quantum systems to a form of
...
\begin{equation}
\op{\rho}_{Q_{1}}\left(t^{\prime}\right)=\Tr_{Q_{2}+\dotsb+Q_{n}}\left[\op{\rho}_{W}\left(t^{\prime}\right)\right]=\sum_{i_{1}}p_{Q_{1}}\left(i_{1};t^{\prime}\right)\ket{\Psi_{Q_{1}}\left(i_{1};t^{\prime}\right)}\bra{\Psi_{Q_{1}}\left(i_{1};t^{\prime}\right)}=\sum_{i_{1}}p_{Q_{1}}\left(i_{1};t^{\prime}\right)\op P_{Q_{1}}\left(i_{1};t^{\prime}\right).\label{eq:SpectralDecompSubsysIntoProjOps}
\end{equation}
...
intuitive, reductionist manner. However, naïve reductionism generically
...
for $W=Q_{1}\equiv Q$: 
\begin{equation}
p_{Q\subset W}\left(j;t^{\prime}\given i;t\right)\to p_{Q}\left(j;t^{\prime}\given i;t\right)\equiv\Tr_{Q}\left[\op P_{Q}\left(j;t^{\prime}\right)\mathcal{E}_{Q}^{t^{\prime}\from t}\left[\op P_{Q}\left(i;t\right)\right]\right]\sim\Tr\left[\op P_{j}\left(t^{\prime}\right)\mathcal{E}\left[\op P_{i}\left(t\right)\right]\right]\ \mathrm{for\ }t^{\prime}-t\gg\delta t_{Q}.\label{eq:ExactOpenSubsysDynMapAfterTimeCoarseGrain}
\end{equation}
...
probabilities do not naïvely compose: We cannot generically express
...
\begin{equation}
\begin{aligned} & \op{\varrho}_{Q\left(t^{\prime}\right)\given Q\left(t\right)}^{Q\subset W}\\
 & =\left(\op 1_{Q\left(t^{\prime}\right)}\tensorprod\op{\rho}_{Q}^{-1/2}\left(t\right)\right)\Tr_{E\left(t^{\prime}\right)}\left[\Tr_{E\left(t\right)}\left[\left(\op 1_{W\left(t^{\prime}\right)}\tensorprod\op{\rho}_{W}^{1/2}\left(t\right)\right)\op{\varrho}_{W\left(t^{\prime}\right)\given W\left(t\right)}\left(\op 1_{W\left(t^{\prime}\right)}\tensorprod\op{\rho}_{W}^{1/2}\left(t\right)\right)\right]\right]\left(\op 1_{Q\left(t^{\prime}\right)}\tensorprod\op{\rho}_{Q}^{-1/2}\left(t\right)\right).
\end{aligned}
...
Von Neumann-Lüders projection \cite{vonNeumann:1955mfqm,Luders:1950udzddm}
...
and features a Von Neumann-Lüders projection postulate \cite{vonNeumann:1955mfqm,Luders:1950udzddm}
...
\subsubsection{Schrödinger's Cat, Wigner's Friend, and the Local Nature of Ontology}



In his famous 1935 thought experiment arguing against the Copenhagen
interpretation \cite{Schrodinger:1935dgsdq},%
\footnote{Schrödinger himself coined the term ``entanglement'' (\emph{Verschränkung})
in this 1935 paper.%
} Schrödinger considered the hypothetical case of an unfortunate cat
...
density matrix 
\[
\op{\rho}_{\mathrm{cat}+\mathrm{WF}}=\frac{1}{2}\ket{\mathrm{alive}}\ket{\mathrm{WF}\left(\quote{\mathrm{alive}}\right)}\bra{\mathrm{alive}}\bra{\mathrm{WF}\left(\quote{\mathrm{alive}}\right)}+\frac{1}{2}\ket{\mathrm{dead}}\ket{\mathrm{WF}\left(\quote{\mathrm{dead}}\right)}\bra{\mathrm{dead}}\bra{\mathrm{WF}\left(\quote{\mathrm{dead}}\right)}.
\]
...
quantum superposition, naïvely assuming (as one does in the Everett-DeWitt
...
Schrödinger intended his thought experiment to be a proof of principle
...
small assemblages of particles, known as ``Schrödinger kittens,''
...
state 
\begin{equation}
\eqsbrace{\begin{aligned}\mathrm{After}\colon\qquad & p_{W}\left(\left(\quote{\mathrm{found\ }A\left(\quote{\uparrow}\right)\binaryand B\left(\quote{\downarrow}\right)\ \emph{twice}},A\left(\quote{\uparrow}\right),B\left(\quote{\downarrow}\right),\uparrow,\downarrow\right)^{\prime}\right)=\frac{1}{2},\\
...
a Von Neumann-Lüders projection (see Appendix~\ref{subsub:The-Copenhagen-Interpretation})
...
\begin{equation}
\op{\rho}_{1+2}\left(\alpha\right)=\frac{1}{12}\ket{+_{1}}\ket{+_{2}}\bra{+_{1}}\bra{+_{2}}+\frac{1}{12}\ket{+_{1}}\ket{-_{2}}\bra{+_{1}}\bra{-_{2}}+\frac{1}{12}\ket{-_{1}}\ket{+_{2}}\bra{-_{1}}\bra{+_{2}}+\frac{9}{12}\ket{-_{1}}\ket{-_{2}}\bra{-_{1}}\bra{-_{2}},\label{eq:MyrvoldDensMatrixAlpha}
\end{equation}
...
might naïvely suggest---that is, the information encoded in their
...
However, according to our interpretation of quantum theory, naïve
...
\end{equation}
 then the system's state vector obeys the famous Schrödinger equation:
\begin{equation}
...
rule called the Von Neumann-Lüders projection postulate \cite{vonNeumann:1955mfqm,Luders:1950udzddm}:
...
eigenstate $\ket{\lambda}$ of $\op{\Lambda}$, the Von Neumann-Lüders
...
The Von Neumann-Lüders projection postulate \eqref{eq:CopInterpVonNeumannLudersProjPost}---known
...
by the Schrödinger equation \eqref{eq:CopInterpSchroEq}, a discrepancy
...
from the Von Neumann-Lüders projection postulate \eqref{eq:CopInterpVonNeumannLudersProjPost}.
...
the Von Neumann-Lüders projection postulate \eqref{eq:CopInterpVonNeumannLudersProjPost}
...
outcome that appears to be in keeping with the Von Neumann-Lüders
...
and the Von Neumann-Lüders projection postulate \eqref{eq:CopInterpVonNeumannLudersProjPost}
...
will experience the Born rule and the Von Neumann-Lüders projection
...
the Von Neumann-Lüders projection postulate. 
