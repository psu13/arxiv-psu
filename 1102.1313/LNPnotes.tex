...
\begin{itemize}
  ...
  \item For each object $A$, an \boldemph{identity} arrow $\id[A]:A\rightarrow A$.
\end{itemize}
...
\mathrm{TY}\ni\, T,U \; ::= \; b \; \mid \; T \rightarrow U \; \mid \; T \times U %&& \text{general types}
\\[1.5mm]
...
\infer{\Gamma \vdash \lambda x. \, t: U \rightarrow T}{\Gamma , x:U \vdash t:T}
\\&&\\&&\\&
\infer{\Gamma \vdash \pi_1 v : T}{\Gamma \vdash v : T \times U} &%
\infer{\Gamma \vdash t\,u : T}{\Gamma \vdash t : U \rightarrow T \qquad \Gamma  \vdash u:U}
\\&&\\&&\\&
...
%    Show then that, for any typed term $\Gamma\vdash t:T$, if $t\rightarrow_\beta t'$ then $\Gamma\vdash t':T$ is typed.
%\end{myexercise}
%
%For example,
%\begin{align*}
%&\iota \rightarrow \iota \rightarrow \iota    \qd[3]\mbox{\it first-order function type} \\
%&(\iota \rightarrow \iota ) \rightarrow \iota \qd[2]\mbox{\it second-order function type}
%\end{align*}
\newlines{1}
...
\\&&\\\hline&&\\
\textbf{Implication} & \infer[\ImpI]{\Gamma\vdash A\supset B}{\Gamma,A\vdash B} & \infer{\Lambda(f):\Gamma\longrightarrow(A\Rightarrow B)}{f:\Gamma
...
\Gamma \longrightarrow (A \Rightarrow B) \quad g : \Gamma \rarr A}\\&&\\
...
%    (It is `most general' in the sense that any other type you could give this term would be of the form $T \rightarrow T$,
