...
\begin{document}
\preprint{hep-th/9401072 \ \ \ IASSNS-HEP-93/87}

\title{On Geometric Entropy}

\author{Curtis Callan\cite{leave} and Frank Wilczek\cite{add}}
\address{
School of Natural Sciences, Institute for
Advanced Study, Princeton, NJ 08540, U.S.A.}
\maketitle
...
The microscopic definition $S~=~-{\rm tr} \rho \ln \rho$ of entropy, where
...
\label{therment}
\end{equation}
where $Z= {\rm tr} e^{-\beta H}$ is the partition function and $H$ is the
...
continuation of ${\rm tr}\rho^n$ to arbitrary $n$ which in turn allows us to
use
...
normalized object $\hat\rho = \rho/{\rm tr}\rho$. Then we can write
\begin{equation}
S_{\rm geom} = -{\rm tr}(\hat\rho {\rm ln} \hat\rho) =
(-{d\over dn} + 1) \ln {\rm tr}\rho^n |_{n=1}~.
\label{replica}
\end{equation}
...
normalization of $\rho$. The identification of ${\rm tr}\rho^n$ as the
partition
...
\pmatrix{\gamma_{++}&\gamma_{+-}\cr\gamma_{-+}&\gamma_{--}}
		\pmatrix{\phi_+\cr \phi_-}\}
\end{equation}
...
\rho(\phi^1_+; \phi^2_+) \propto
	\exp\{-{1\over 2}(\phi_+^1~ \phi_+^2) \pmatrix{A&2B\cr 2B&A}
	\pmatrix{\phi_+^1\cr \phi_+^2}\}
\label{gaussdm}
\end{equation}
where
\begin{eqnarray}
A &= 2(\gamma_{++} -{1 \over 2} \gamma_{+-} (\gamma_{--})^{-1} \gamma_{-+})\cr
B &= -{1\over2} \gamma_{+-} (\gamma_{--})^{-1} \gamma_{-+}\cr
\label{ABdm}
\end{eqnarray}
where the inverse kernels are inverses of the restricted kernels, {\it not}
restrictions of the full $\Gamma^{-1}$.

Using Eq.\ref{gaussdm}, we can write ${\rm tr}\rho^n$ as a functional integral:
\begin{equation}
{\rm tr}\rho^n =
\int [{\cal D} \phi_+] \exp
 \Biggl\{ -(\phi^1_+ \cdots \phi^n_+) {\cal M}_n
\pmatrix{\phi^1_+\cr \vdots\cr \phi^+_n \cr} \Biggr\}
\end{equation}
where ${\cal M}_n$ is the supermatrix
\begin{equation}
{\cal M}_n = \pmatrix{A & B & 0 & \ldots & 0 & B \cr
B & A & B & \ldots & 0 & 0 \cr
0 & B & A & \ldots & 0 & 0 \cr
...
functional integral gives the result ${\rm tr}\rho^n =1/\sqrt{{\rm det}{\cal
M}_
 n}$.
...
\begin{equation}
\zeta(t)= {\rm tr}(e^{t\Delta})=\sum_n e^{-t\lambda_n}.
\end{equation}
...
1. There are also finite quantities lurking within this circle of ideas which
...
2. String theory is thought to provide a consistent framework for
...
3. How should one react to the divergence of the first quantum correction
...
\begin{references}
\bibitem[\dagger]{leave}On leave from Princeton University. Research supported
in part by DOE grant DE-FG02-90ER40542 and by the Monell Foundation.
\bibitem[*]{add}Research supported in part by DOE
